...
def test_slow(func, *args, flag):
    try:
        measure = timeit(
            lambda: hashlib.md5(str(random.randint(0, 2**24)).encode()),
            number=10000)
        wait = measure * 80
...
def marvelous_regex():
    ...
    if len(s) > 1000:
        ...
        exit()
    pattern = re.compile(
        r'^Dear Sophia, (.*柴魚){10,15}.*\. Best wishes, ([a-zA-Z0-9]+ ?)+\.$')
    if test_slow(pattern.match, s, flag=flag):
        print(
            'Okay, got your message. But Sophia fed her 柴魚 with your letter QQ'
        )
    else:
